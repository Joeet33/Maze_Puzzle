...
class Cell(IntEnum):
    # empty cell where the agent can move to
    EMPTY = 0
    # wall that cannot be entered
    OCCUPIED = 1
...
class Maze:
    ...
    # reward for getting to the finish
    reward_exit = 10.0
    # penalty for every move that does not reach the finish
    penalty_move = -0.05
    ...
    def __init__(self, maze, start_cell=(0, 0), exit_cell=None):
        ...
        self.maze = maze
        # reset game if accumulated reward is below this threshold
    ...
    def __draw(self):
        # draws a line from the agents previous cell to its current cell
    ...
    def step(self, action):
        # moves the agent according to 'action' and returns the new state, reward and game status
    ...
    def __execute(self, action):
        # executes action and collect the reward or penalty
        possible_actions = self.__possible_actions(self.__current_cell)

        if not possible_actions:
            # if stuck, force end of game
            ...
            if self.__current_cell == self.__exit_cell:
                # maximum reward when reaching the exit cell
    ...
    def __possible_actions(self, cell=None):
        # creates a list with all possible actions from cell, avoiding the maze's edges and walls
        if cell is None:
            col, row = self.__current_cell
        else:
            col, row = cell
        ...
        # restrict the initial list by removing impossible actions
        nrows, ncols = self.maze.shape
    ...
    def __observe(self):
        # returns the state of the maze - in this game the agents current location

        return np.array([[*self.__current_cell]])

    def play(self, model, start_cell=(0, 0)):
        # plays a single game, choosing the next move based a prediction from 'model'
        self.reset(start_cell)
    ...
    def check_win_all(self, model):
        # checks if the model wins from all possible starting cells
        ...
        for cell in self.empty:
            if self.play(model, cell) == Status.WIN:
                ...
                lose += 1
        # restores previous rendering setting
    ...
    def render_q(self, model):
        # renders the recommended action(s) for each cell
        def clip(n):
